...
const inputStateReducer = (state, action) => {
  ...
  if (action.type === "INITIALIZE") return { value: action.value, isTouched: true };
...
const useInput = (validate) => {
  ...
  const changeValue = (event) => dispatch({ type: "INPUT", value: event.target.value.id || event.target.value });
  const initializeValue = (value) => dispatch({ type: "INITIALIZE", value });
  const blur = () => dispatch({ type: "BLUR" });
  const resetValue = () => dispatch({ type: "RESET" });

  return { value: inputState.value, isValid, error, changeValue, initializeValue, blur, resetValue };
};
