    ...
    <Grid container>
      ...
      <Grid item xs={12} sm={12} md={10} lg={10} className={style.second}>
        ...
        <div className={`${defaultStyle.center} ${defaultStyle.marginTop}`}>
          <Button type="button" color="success" size="small" onClick={() => setStep((previousStep) => previousStep - 1)} disabled={step === 0}><KeyboardArrowLeft /></Button>
          {`${step + 1} / ${length}`}
          <Button type="button" color="success" size="small" onClick={() => setStep((previousStep) => previousStep + 1)} disabled={(step + 1) === length}><KeyboardArrowRight /></Button>
        </div>
      </Grid>
    </Grid>
  );
